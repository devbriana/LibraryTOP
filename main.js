...
function displayBooks() {
  ...
  // card content
    cardDiv.innerHTML = `
      ...
      <div class="card-actions">
        <button class="pages-hover" aria-label="View Pages Read" data-book-id="${book.id}"><img src="icons/pagesHover.svg" alt="Pages read"></button>
  ...
  pageHoverIcons.forEach(icon => {
    icon.addEventListener('mouseenter', () => {
      const bookId = icon.dataset.bookId; // Get the book's ID
      const book = myLibrary.find(b => b.id === bookId);

      if (book) {
        // Display quickInfo in a simple dialog box
        const infoDialog = document.createElement('dialog');
        infoDialog.id = "book-info-dialog"; 
        infoDialog.textContent = book.quickInfo();
        document.body.appendChild(infoDialog);
        infoDialog.show();

        // Close dialog automatically when mouse leaves
